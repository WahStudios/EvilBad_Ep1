...
public class TileEditorWindow : EditorWindow {
	...
	void OnGUI () {
		...
		EditorGUILayout.BeginVertical();
			...
			if(this.prefab!=null) {
				this.prefabOffset = EditorGUILayout.Vector3Field("Prefab offset", this.prefabOffset);
			} else {
				 this.prefabOffset = new Vector3(this.layer.tileSpacing.x / 2f, this.layer.tileSpacing.y / 2f, 0);
			}
	...
	public void Setup(TileLayerEditor editor, TileInstance[] selectedTiles) {
		...
		if(layer.tileset!=null) {
			...
			if(tile!=null) {
				...
				if(this.prefab == null) this.prefabOffset = new Vector3(this.layer.tileSpacing.x / 2f, this.layer.tileSpacing.y / 2f);
